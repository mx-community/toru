...
const sendEpisodes = async (conn, m, episodes, dramaName) => {
  if (episodes.length > 1 && conn.sendSylphy) {
    const medias = episodes.map((ep) => ({
      type: 'video',
      data: { url: ep.mp4 },
    ...
    for (const chunk of chunks) {
      await conn.sendSylphy(m.chat, chunk, { quoted: m })
    }
  } else {
...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  if (isUrl && args.length > 1) {
    ...
    try {
        ...
        if (range.toLowerCase() === 'all') {
           return m.reply('⚠️ "all" es demasiado. Usa rangos de 5 caps (ej: 1-5).')
        } else {
        ...
        if (!targets.length) return m.reply('❎ No encontré episodios en ese rango.')
        
        if (targets.length > 10) return m.reply('⚠️ Máximo 10 episodios por comando.')
  ...
  if (isUrl) {
    m.reply('⏳ *Obteniendo información del drama...*')
    try {
        ...
        if (!targets.length && list.length > 0) {
            let txt = `📺 *${dramaName}*\n`
            txt += `📂 Total Episodios: ${data.bookInfo?.totalChapterNum || list.length}\n`
            txt += `📝 ${data.bookInfo?.synopsis || ''}\n\n`
            txt += `👇 *Responda con el número del episodio (o rango) que desea descargar.*\n`
            txt += `Ej: *1* o *1-5*`
  ...
  if (!text) return m.reply(`*Ingrese el nombre del drama.*\nEjemplo: ${usedPrefix + command} my boss`)

  m.reply('🔎 *Buscando en DramaBox...*')
  try {
      const results = await searchDrama(text)
      if (!results.length) return m.reply('❎ No encontrado.')
      
      let txt = `🎥 *RESULTADOS DRAMABOX*\n\n`
      results.slice(0, 10).forEach((b, i) => {
          txt += `*${i + 1}.* ${b.bookName} (${b.totalChapterNum} eps)\n`
      })
...
handler.before = async (m, { conn }) => {
    ...
    if (session.type === 'search') {
        ...
        m.reply('⏳ *Obteniendo episodios...*')
        try {
            const data = await fetchInfo(link)
            const list = data.chapterList || []
            
            let txt = `📺 *${item.bookName}*\n`
            txt += `📂 Total Episodios: ${data.bookInfo?.totalChapterNum || list.length}\n`
            txt += `📝 ${data.bookInfo?.synopsis || ''}\n\n`
            txt += `👇 *Responda con el número del episodio (o rango) que desea descargar.*\n`
            txt += `Ej: *1* o *1-5*`
    ...
    if (session.type === 'details') {
        ...
        try {
             ...
             if (range.toLowerCase() === 'all') {
                return m.reply('⚠️ "all" es demasiado. Usa rangos de 5 caps.')
             } else {
                 range.split(',').forEach(p => {
                     if (p.includes('-')) {
                         const [a, b] = p.split('-').map(x => parseInt(x.trim()))
             ...
             const targets = list.filter(c => e.has(c.index + 1))
             if (!targets.length) return m.reply('❎ No encontré episodios en ese rango.')
             if (targets.length > 5) return m.reply('⚠️ Máximo 5 episodios para evitar spam. Usa rangos pequeños.') // Changed from 10 to 5 for safety
...
handler.help = ['dramabox <query>', 'dramabox <url> <rango>']
handler.tags = ['search', 'downloader']
handler.command = /^(dramabox|dbox|dramaboxdb)$/i
