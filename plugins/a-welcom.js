...
return { pp, bienvenidaXd, mentions: [userId] }
}
...
return { pp, despedidaXd, mentions: [userId] }
}
...
const { pp, bienvenidaXd, mentions } = await generarBienvenida({ conn, userId, groupMetadata, chat })
const thumbWel = Buffer.from(await (await fetch(`${global.toruImg}`)).arrayBuffer())
await conn.sendMessage(m.chat, { text: bienvenidaXd, mentionedJid: mentions, contextInfo: { externalAdReply: { title: "〩 ¡Bienvenido!", body: botname, thumbnail: thumbWel, sourceUrl: null, mediaType: 1, renderLargerThumbnail: false }}}, { quoted: fkontak })
}
...
const thumbBye = Buffer.from(await (await fetch(`${global.toruImg}`)).arrayBuffer())
await conn.sendMessage(m.chat, { text: despedidaXd, mentionedJid: mentions, contextInfo: { externalAdReply: { title: "〩 ¡Despedida!", body: botname, thumbnail: thumbBye, sourceUrl: null, mediaType: 1, renderLargerThumbnail: false }}}, { quoted: fkontak })
}
